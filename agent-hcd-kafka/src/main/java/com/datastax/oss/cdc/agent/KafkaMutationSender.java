...
/**
 * Kafka-backed {@link MutationSender} for HCD (Hybrid Cloud Distribution).
 *
...
public class KafkaMutationSender extends AbstractKafkaMutationSender<TableMetadata> {
    ...
    Object convertToAvro(AbstractType<?> type, Schema avroSchema, Object value) {
        ...
        if (type instanceof UserType) {
            ...
            ByteBuffer[] fieldBufs = udt.split(ByteBufferAccessor.instance, rawUdt);
        ...
        if (type instanceof TupleType) {
            ...
            ByteBuffer[] fieldBufs = tuple.split(ByteBufferAccessor.instance, rawTuple);
