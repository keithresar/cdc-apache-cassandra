...
public class Agent {
    ...
    static void startCdcAgent(String agentArgs) throws Exception {
        ...
        log.info("Starting Kafka CDC agent v{}, cdc_raw_directory={}", agentVersion,
                DatabaseDescriptor.getCDCLogLocation());
        AgentConfig config = AgentConfig.create(AgentConfig.Platform.KAFKA, agentArgs);
        ...
        CommitLogReaderServiceImpl commitLogReaderService = new CommitLogReaderServiceImpl(
                config, kafkaMutationSender, segmentOffsetFileWriter, commitLogTransfer);
        // DSE4: getCDCLogLocation() returns File — use getAbsolutePath()
        CommitLogProcessor commitLogProcessor = new CommitLogProcessor(
                DatabaseDescriptor.getCDCLogLocation().getAbsolutePath(), config, commitLogTransfer,
                segmentOffsetFileWriter, commitLogReaderService, true);
        ...
        log.info("Kafka CDC agent started");
    }
}
